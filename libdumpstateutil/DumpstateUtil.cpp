...
bool PropertiesHelper::IsUserBuild() {
    if (build_type_.empty()) {
        build_type_ = "user"; // android::base::GetProperty("ro.build.type", "user");
    }
...
bool PropertiesHelper::IsDryRun() {
    if (dry_run_ == -1) {
        dry_run_ = 0; // android::base::GetBoolProperty("dumpstate.dry_run", false) ? 1 : 0;
    }
    return dry_run_ == 1;
}
...
int RunCommandToFd(int fd, const std::string& title, const std::vector<std::string, std::allocator<std::string>>& full_command,
