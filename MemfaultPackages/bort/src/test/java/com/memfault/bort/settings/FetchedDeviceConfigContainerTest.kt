...
class FetchedDeviceConfigContainerTest {
                ...
                "memfault": {
                    "bort": {
                        "sdk-settings": {
                            ...
                            "http_api.device_base_url": "http://app.memfault.test:8000",
                            "http_api.files_base_url": "http://app.memfault.test:8000",
    ...
    private val responseWithDataUploadStartDate = """
    {
        "data": {
            "revision": 3344,
            "config": {
                "memfault": {
                    ...
                    "data_upload_start_date": "PLACEHOLDER"
                },
                "customer": {
                    "prop1": "val1"
                }
    ...
    @Test
    fun decodeDataUploadStartDateNull() {
        val result = BortJson.decodeFromString(DecodedDeviceConfig.serializer(), response)
        assertThat(result.memfault?.dataUploadStartDate).isNull()
    }

    @Test
    fun decodeDataUploadStartDateRFC3339Compliant() {
        val result = BortJson.decodeFromString(
            DecodedDeviceConfig.serializer(),
            responseWithDataUploadStartDate.replace("PLACEHOLDER", "1989-05-29T10:40:12+00:00"),
        )
        assertThat(result.memfault?.dataUploadStartDate)
            .isEqualTo(Instant.parse("1989-05-29T10:40:12+00:00"))
    }

    @Test
    fun decodeDataUploadStartDateInvalid() {
        val result = BortJson.decodeFromString(
            DecodedDeviceConfig.serializer(),
            responseWithDataUploadStartDate,
        )
        assertThat(result.memfault?.dataUploadStartDate).isNull()
    }

    @Test
    fun decodeDataUploadStartDateRFC3339NonCompliant() {
        val result = BortJson.decodeFromString(
            DecodedDeviceConfig.serializer(),
            responseWithDataUploadStartDate.replace("PLACEHOLDER", "1989-05-29T10:40:12+00:00Z"),
        )
        assertThat(result.memfault?.dataUploadStartDate).isNull()
    }
}
