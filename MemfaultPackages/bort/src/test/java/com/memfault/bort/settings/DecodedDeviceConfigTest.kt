...
class DecodedDeviceConfigTest {

    @Test
    fun `deserialize with check unknown fields`() {
        ...
        """
        // The json file above should not be touched as it's a test for forwards compatibility. If you need
        // to add to this file then your change is breaking future compatibility.
        assertThat(json).hasLength(8250)

        val config = DecodedDeviceConfig.from(json)
        assertThat(config.revision).isEqualTo(0)
        assertThat(config.completedRevision).isEqualTo(0)
        assertThat(config.memfault?.sampling).isNotNull()
        assertThat(config.others).isNotNull()
    }
}
